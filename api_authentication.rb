...
def api_authenticate!
	...
	if bearer
		...
		@api_user = User.get(user_id)
		 	...
		 	halt 401, {message: 'A valid token must be passed.'}.to_json
	    rescue JWT::ExpiredSignature
	     	halt 401, {message: 'The token has expired.'}.to_json
	    rescue JWT::InvalidIssuerError
	    	halt 401, {message: 'The token does not have a valid issuer.'}.to_json
	    rescue JWT::InvalidIatError
	    	halt 401, {message: 'The token does not have a valid "issued at" time.'}.to_json
		end
	else
		halt 401, {message: 'A valid token must be passed.'}.to_json
	end
